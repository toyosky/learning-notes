...
> 用 akshare 替代 yfinance，获取沪深300、纳指100、黄金 ETF 数据。当前数据源为**新浪**，腾讯源作为容灾。

## 为什么不用 yfinance？

yfinance 请求雅虎财经接口有**频率限制**（`YFRateLimitError`），而且对中国 A 股支持不好。改用 akshare，速度更快、更稳定。

`data_fetcher.py` 的数据源策略：

| 优先级 | ETF | A 股 |
|--------|-----|------|
| 1️⃣ 新浪（主力） | `fund_etf_hist_sina` 手动拆股 | `stock_zh_a_daily` 原生 qfq |
| 2️⃣ 腾讯（容灾） | `stock_zh_index_daily_tx` 无复权 | `stock_zh_index_daily_tx` 无复权 |

当前新浪接口可达，数据统一来自**新浪源**。不使用东方财富。
...
> **`.SS` / `.SZ` 是什么？** `.SS` 代表上海交易所，`.SZ` 代表深圳交易所。yfinance、新浪等数据源需要在代码后加后缀（如 `510300.SS`），但 `data_fetcher.py` 内部**自动添加**，调用时传纯数字即可。
>
> **当前数据源说明**：本笔记所有示例数据均来自**新浪源**，腾讯源作为容灾回退（无复权）。删除本地 parquet 缓存即可强制重拉。
