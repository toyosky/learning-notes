...
> 完整的数学推导、优化原理（二次规划/拉格朗日乘子/解析解）和 MPT 局限的详细分析见独立笔记：
> [[../deep/efficient-frontier-derivation|有效前沿数学推导 — 从组合方差到最优权重]]

### 核心概念

有效前沿源自 **马科维茨（Markowitz, 1952）现代投资组合理论（MPT）**：

> **组合风险不是单个资产风险的加权平均** — 资产间的**相关性**才是关键。有效前沿就是一系列（波动率, 收益）组合点的上包络线：每个点上，**给定风险水平收益最高**，或**给定收益水平风险最低**。

在我们的三资产案例中，日收益率相关系数均在 0.3 以下，这是组合效果出色的数学基础：
- $\rho_{沪深300,\ 纳指100} = 0.26$
- $\rho_{沪深300,\ 黄金} = 0.05$
- $\rho_{纳指100,\ 黄金} = 0.03$

$$E(R_p) = \mathbf{w}^T \boldsymbol{\mu}, \quad \sigma_p^2 = \mathbf{w}^T \boldsymbol{\Sigma} \mathbf{w}$$

当 $\rho < 1$ 时，组合方差**总是小于**各资产方差的加权平均 — 分散化收益来自不完全正相关。
...
- 前沿的左端比右端更密集 — 在低波动区域不同权重的风险差异不大，这是三资产相关性低的效果
- **在实践中**：鉴于估计误差（详细分析见推导笔记 §9），简单等权组合是比最优权重更可靠的选择
