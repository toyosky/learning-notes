...
在风险平价（每月再平衡）基础上加入相同的回撤止损规则，结果完全不同：

| 方案 | 最终值 | 累计收益 | 年化 | 最大回撤 | 夏普 | 触发次数 |
|------|--------|---------|------|---------|------|---------|
| **无止损失** | ¥128,450 | +28.45% | 5.34% | 16.25% | 0.25 | — |
| **5% 止损** | ¥119,338 | +19.34% | 3.74% | 17.10% | 0.11 | 86 |
| **10% 止损** | ¥126,112 | +26.11% | 4.94% | 17.78% | 0.21 | 18 |
| **15% 止损** | **¥180,855** | **+80.86%** | **13.11%** | **15.68%** | **0.63** | **1** |

**15% 止损的结果极为突出（+80.9% vs +28.5%），仅触发 1 次。**

这与动量排名的情况完全相反——动量排名里 15% 从未触发（回撤最大 13.2%），而对风险平价而言，15% 阈值刚好够宽，只拦截最极端的行情。这个单次触发大概率发生在 2022 年股债双杀的环境下——风险平价的最薄弱场景：

- **普通下跌**：风险平价的分散化起作用，股债对冲，回撤可控
- **股债双杀**：所有资产同时跌，分散化失效，回撤急剧扩大
- **15% 止损正好在这种情景触发**：清仓 → 现金 → 下月再平衡日重新入场，避开了最陡峭的下跌段

**5% 和 10% 对风险平价无效**，因为风险平价的资产本身波动率高（纳指 20%+ 年化波动），正常波动就能触发，86 次和 18 次的频繁离场严重损害收益。

| 对比维度 | 动量排名 | 风险平价 |
|---------|---------|---------|
| 最佳止损阈值 | 10% | 15% |
| 提升幅度 | +0.4pp 收益，+0.01 夏普 | **+52.4pp 收益，+0.38 夏普** |
| 触发频率 | 4次/5年 | 1次/5年 |
| 为何有效 | 避开趋势反转初期的急跌 | 避开股债双杀——分散化失效时刻 |
